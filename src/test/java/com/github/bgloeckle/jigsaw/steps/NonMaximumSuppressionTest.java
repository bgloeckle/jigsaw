...
public class NonMaximumSuppressionTest {
    private static final Supplier<InputStream> FOREST_ROAD_EXPECTED = () -> NonMaximumSuppressionTest.class
                    .getResourceAsStream("/" + NonMaximumSuppressionTest.class.getSimpleName()
    ...
    @Test
    public void gaussian5() throws IOException {
        Pipeline p = new Pipeline(new ToSimpleLuminosityGreyscale(), new GaussianBlur(5), new SobelFilter(),
                        new NonMaximumSuppression(), new FromSimpleLuminosityGreyscale());
        img = p.process(img);
